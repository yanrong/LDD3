...
int jit_fn_show(struct seq_file *m, void *v)
{
    ...
    j1 = jiffies; /* actual value after we delay */
    seq_printf(m, "%9li %9li\n", j0, j1);
    return 0;
}

static int jit_fn_open(struct inode *node, struct file *file)
{
    return single_open(file, jit_fn_show, PDE_DATA(node));
}
...
static int jit_currentime_open(struct inode *node, struct file *file)
{
...
/* This data struture used as "data" for the timer and tasklet functions */
struct jit_data {
...
int jit_timer_show(struct seq_file *m, void *v)
{
    ...
    /* fill the data for four timer function */
    data->prevjiffies = j;
    data->m = m;
...
static int jit_timer_open(struct inode *node, struct file *file)
{
...
static int jit_tasklet_open(struct inode *node, struct file *file)
{
    return single_open(file, jit_tasklet_show, PDE_DATA(node));
}
