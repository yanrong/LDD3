...
static void tiny_timer(struct timer_list *t)
{
	...
	for (i = 0; i < data_size; i++) {
		if (!tty_buffer_request_room(port, 1)) // the current buffer is full
			tty_flip_buffer_push(port);
		tty_insert_flip_char(port, data[i], TTY_NORMAL);
	}
	tty_flip_buffer_push(port);

	/* resubmit the timer agian */
...
static int tiny_open(struct tty_struct *tty, struct file *filp)
{
	...
	if (tiny == NULL) {
		/* first time access this device, let's create it */
	...
	/* save out structure within the tty structure */
...
static void tiny_set_termios(struct tty_struct *tty, struct ktermios *old_termios)
{
	...
	/* check the they really want us to change something */
	if (old_termios) {
	...
	/* figure out the stop bit requested */
	if (cflag & CSTOPB)
...
static int __init tiny_init(void)
{
	...
	if (retval) {
		...
		for (j = 0; j < i; j++)
			tty_port_destroy(tiny_tty_port + j);
		return retval;
	}
