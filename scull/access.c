...
#include <linux/cred.h> /* current_uid() and current_euid() */
...
{
    atomic_inc(&scull_s_available); /* release the devices */
    return 0;
}
...
static struct scull_dev *scull_c_lookfor_device(dev_t key)
{
    ...
    lptr->key = key;
    scull_trim(&(lptr->device)); /* intialize it */
