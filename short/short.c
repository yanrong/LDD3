...
static int probe = 0; /* select at load time how to probe_irq line */
...
struct timespec64 tv_data[NR_TIMEVAL];
...
#endif
{
    ...
    do {
        written = sprintf((char *)short_head, "%08u.%06lu\n",
            (int)(tv_tail->tv_sec % 100000000), (int)tv_tail->tv_nsec / NSEC_PER_USEC);
...
void short_selfprobe(void)
{
     ...
     */
    for (i = 0; trials[i]; i++)
        tried[i] = request_irq(trials[i], short_probing, 0,
            "short probe", NULL);
...
int short_init(void)
{
    ...
    if (!use_mem) {
        if (!request_region(short_base, SHORT_NR_PORTS, "short")) {
            printk(KERN_INFO "short: can't get IO port address 0x%lx\n", short_base);
            return -ENODEV;
        }
    } else {
        if (!request_mem_region(short_base, SHORT_NR_PORTS, "short")) {
            printk(KERN_INFO "short: can't get IO mem address 0x%lx\n", short_base);
            return -ENODEV;
        }
    ...
    /* Here we register our device - should not failed thereafter */
    ...
    if (short_irq >= 0) {
        result = request_irq(short_irq, short_interrupt, 0,
                "short", NULL);
        if (result) {
            printk(KERN_INFO "short: can't get assigned irq %i\n", short_irq);
            short_irq = -1;
...
void short_cleanup(void)
{
    ...
    if (use_mem) {
        ...
        // release_mem_region(short_base, SHORT_NR_PORTSS);
